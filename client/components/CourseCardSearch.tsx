...
const CourseCardSearch = ({ course, isSelected, onClick }: SearchCourseCardProps) => {
    return (
        <div onClick={onClick} className={`course-card-search group ${isSelected ? "course-card-seach--selected" : "course-card-search--unselected"}`}>
            <div className='course-card-search__image-container'>
                <Image src={course.image || "/placeholder.png"} alt={course.title} fill sizes="(max-width:768px) 100vw, (max-width:1220px) 50vw, 33vw" className='course-card-search__image' priority />
            </div>
            ...
            <div className='course-card-search__content'>
                ...
                <div className='mt-2'>
                    <p className='course-course-card-search__teacher'>
